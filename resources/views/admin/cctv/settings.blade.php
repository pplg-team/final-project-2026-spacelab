<x-app-layout title="Pengaturan Kamera CCTV" description="Atur tipe kamera dan stream URL per ruangan">
    ...
    <div class="py-6">
        <div class="space-y-6">
            ...
            @if (session('success'))
                <div class="bg-green-50  border border-green-200  lg p-4">
                    <div class="flex items-center">
                        <x-heroicon-o-check-circle class="w-5 h-5 text-green-600  mr-3 flex-shrink-0" />
            ...
            @if ($errors->any())
                <div class="bg-red-50  border border-red-200  lg p-4">
                    <div class="flex items-start">
                        <x-heroicon-o-exclamation-triangle class="w-5 h-5 text-red-600  mr-3 mt-0.5 flex-shrink-0" />
                        <div class="flex-1">
            ...
            <div class="bg-white  shadow-sm sm:lg border border-gray-200  overflow-hidden">
                <div class="p-6 border-b border-gray-200 ">
                    <h3 class="text-lg font-medium text-gray-900 ">Daftar Kamera per Ruangan</h3>
                    <p class="mt-1 text-sm text-gray-600 ">Data diambil langsung dari database ruangan.</p>
                </div>

                <div class="p-4 border-b border-gray-200  bg-gray-50/70 ">
                    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3">
                        <div class="xl border border-blue-200  bg-blue-50  p-4">
                            <p class="text-xs font-semibold uppercase tracking-wider text-blue-700 ">Total Hasil Filter</p>
                            <p class="mt-2 text-2xl font-bold text-blue-900 ">{{ $totalFiltered }}</p>
                        </div>
                        <div class="xl border border-green-200  bg-green-50  p-4">
                            <p class="text-xs font-semibold uppercase tracking-wider text-green-700 ">Kamera Aktif</p>
                            <p class="mt-2 text-2xl font-bold text-green-900 ">{{ $activeCount }}</p>
                        </div>
                        <div class="xl border border-indigo-200  bg-indigo-50  p-4">
                            <p class="text-xs font-semibold uppercase tracking-wider text-indigo-700 ">IP Camera</p>
                            <p class="mt-2 text-2xl font-bold text-indigo-900 ">{{ $ipCameraCount }}</p>
                        </div>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200 ">
                        <thead class="bg-gray-50 ">
                            <tr>
                                <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-600  w-16">No</th>
                                <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-600  min-w-[220px]">Ruangan</th>
                                <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-600  min-w-[180px]">Gedung</th>
                                <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-600  min-w-[180px]">Tipe Kamera</th>
                                <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-600  min-w-[260px]">URL Stream</th>
                                <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-600  min-w-[130px]">Status</th>
                                <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-gray-600  min-w-[140px]">Aksi</th>
                            </tr>
                        </thead>
                            ...
                            @forelse ($rooms as $room)
                                ...
                                <tr class="hover:bg-gray-50  align-top">
                                    ...
                                    <td class="px-4 py-4">
                                        ...
                                        <p class="text-xs text-gray-500  mt-1">{{ $room->code }}</p>
                                    </td>
                                    ...
                                    <td class="px-4 py-4">
                                        <input type="url" name="stream_url" form="{{ $formId }}" value="{{ $roomStreamUrl }}" placeholder="http://192.168.1.x/stream"
                                            class="w-full md border-gray-300    shadow-sm focus:border-gray-500  focus:ring-gray-500  text-sm">
                                        <p class="mt-1 text-xs text-gray-500 ">Wajib untuk tipe IP Camera.</p>
                                    </td>
                                    <td class="px-4 py-4">

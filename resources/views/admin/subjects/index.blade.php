<x-app-layout>
    ...
    <div class="py-8">
        <div class="mx-auto">
            ...
            <!-- Subjects Table -->
            <div class="bg-white  xl border border-gray-200  overflow-hidden">
                @forelse ($subjects as $subject)
                        ...
                        <table class="min-w-full divide-y divide-gray-200 ">
                            <thead class="bg-gray-50 ">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-700  uppercase tracking-wider">Nama Mata Pelajaran</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-700  uppercase tracking-wider">Tipe</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-700  uppercase tracking-wider">Jurusan Diizinkan</th>
                    ...
                    @endif
                                <tr class="hover:bg-gray-50  transition-colors">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="inline-flex px-2.5 py-0.5 text-xs font-medium bg-blue-100  text-blue-800 ">
                                        ...
                                        @if ($subject->allowedMajors->isNotEmpty())
                                            <div class="flex flex-wrap gap-1">
                                                @foreach ($subject->allowedMajors->take(3) as $allowed)
                                                    <span class="inline-flex px-2 py-0.5 text-xs font-medium  bg-green-100  text-green-800 ">
                                                        {{ $allowed->major->code }}
                                                    </span>
                                                @endforeach
                                                @if ($subject->allowedMajors->count() > 3)
                                                    <span class="text-xs text-gray-500 ">+{{ $subject->allowedMajors->count() - 3 }}</span>
                                                @endif
                                            </div>
                                        @else
                                        ...
                                        @if ($subject->teachers->isNotEmpty())
                                            <div class="flex flex-wrap gap-1">
                                                @foreach ($subject->teachers->take(2) as $teacher)
                                                    <span class="inline-flex px-2 py-0.5 text-xs font-medium  bg-purple-100  text-purple-800 ">
                                                        {{ Str::limit($teacher->user->name, 20) }}
                                                    </span>
                                                @endforeach
                                                @if ($subject->teachers->count() > 2)
                                                    <span class="text-xs text-gray-500 ">+{{ $subject->teachers->count() - 2 }}</span>
                                                @endif
                                            </div>
                                        @else
                                            <span class="text-xs text-gray-500 ">-</span>
    ...
    <x-modal name="subjectModal" :show="false" focusable>
        <form id="subjectForm" method="POST" action="{{ route('admin.subjects.store') }}" class="p-6">
            ...
            <div class="space-y-5">
                ...
                <div>
                    <label for="allowedMajors" class="block text-sm font-medium text-gray-700  mb-1.5">
                        Jurusan yang Diizinkan <span class="text-gray-400 text-xs">(Opsional)</span>
                    </label>
                    <div class="max-h-48 overflow-y-auto border border-gray-300  lg p-3 space-y-2 bg-gray-50 ">
                        @forelse ($majors as $major)
                            <label class="flex items-center cursor-pointer hover:bg-white  p-2  transition-colors">
                        ...
                        @empty
                            <p class="text-sm text-gray-500 ">Belum ada jurusan</p>
                        @endforelse
                    </div>
                </div>

...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function clearSegmentInfo(
    message = "Pilih blok jam berwarna hijau pada timeline untuk melihat detail segmen.",
  ) {
    if (segmentEmptyHint) {
  ...
  function playSegment(segment) {
    ...
    if (segmentStart)
      segmentStart.textContent = startTime.toLocaleTimeString("id-ID");
    if (segmentEnd)
      segmentEnd.textContent = endTime.toLocaleTimeString("id-ID");
    if (segmentDuration)
      segmentDuration.textContent = formatDuration(segment.duration_seconds);
    if (segmentSize)
      segmentSize.textContent = formatFileSize(segment.file_size_bytes);
    ...
    if (segmentResolution)
      segmentResolution.textContent = segment.resolution || "-";
    if (segmentIntegrity)
      segmentIntegrity.textContent = segment.integrity_status || "-";
